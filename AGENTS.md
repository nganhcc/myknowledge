...
- Gemini Embedding 2 defaults to 3072 dimensions. Keep `EMBEDDING_DIMENSION`, the Gemini `embedContentConfig.output_dimensionality`, `DocumentChunk.embedding`, and the initial migration aligned at 768; validate API response dimensions before persisting.
- If document processing fails during SQLAlchemy flush/commit, call `rollback()` and reload the document before updating retry state; the failed session transaction cannot be used directly.
- Send Gemini API keys in the `x-goog-api-key` header, not the URL query string; HTTP request logging otherwise exposes the credential.
